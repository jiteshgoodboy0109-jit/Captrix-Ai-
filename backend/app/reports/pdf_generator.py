...
def generate_pdf_report(company_name: str, statements: Dict[str, Any], ratios: Dict[str, Any], corp_fin: Dict[str, Any], ai_reports: Dict[str, Any]) -> bytes:
    ...
    score_color = "#16A34A" if (score_val is not None and score_val >= 75) else ("#D97706" if (score_val is not None and score_val >= 55) else "#DC2626")
    score_str = f"{score:.1f} / 100" if score_val is not None else str(score)
    story.append(Paragraph(f"Overall Financial Health Score: <b><font color='{score_color}'>{score_str}</font></b>", h2_style))
    ...
    left_column_items = [
        ...
        f"&nbsp;&nbsp;Accumulated depreciation: ({fmt(abs(float(ppe.get('accumulated_depreciation') or 0)))})",
    ...
    right_column_items = [
        ...
        f"&nbsp;&nbsp;Less: Treasury stock: ({fmt(abs(float(eq.get('treasury_stock') or 0)))})",
